...
`system_settings`. Отдельный worker раз в минуту проверяет панель. Если удалено только сообщение,
он создаёт и закрепляет его заново. Если Telegram подтверждает отсутствие всей темы, service под
lock создаёт замену, сохраняет новый ID, сбрасывает недействительные временные формы и повторно
создаёт панель уже в новой теме. Единственный повтор ограничивает восстановление от циклов при
других Telegram-ошибках.
