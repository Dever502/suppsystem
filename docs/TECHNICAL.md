...
Telegram entities типа `hashtag`; семантического анализа, словарей и ML нет. Backend также
проверяет каждое использование `#` в сыром тексте: тег должен иметь формат `#текст` без
пробела, содержать букву или цифру и совпадать с распознанной Telegram entity. При 0–5 корректных
хештегах исходный текст и список тегов записываются в `quick_responses`. Затем бот публикует одно
