...
Состояние панели проверяется раз в минуту. Удалённое сообщение панели создаётся и закрепляется
заново; при удалении всей темы бот создаёт новую тему и панель, сохраняя группы и ответы из БД.
Незавершённые временные формы сбрасываются. Историю сообщений удалённой Telegram-темы восстановить
невозможно.
