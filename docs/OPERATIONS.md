...
При старте бот создаёт отдельную системную тему `📚 Готовые ответы`. Только внутри неё доступны:

| Команда | Действие |
| --- | --- |
| `/addanswer Название` + текст со следующей строки | сохранить ответ и один раз опубликовать его карточку |
| `/answers` | открыть персональный каталог с пагинацией и preview |

Пример создания:

```text
/addanswer AI не отвечает
Понимаем проблему. Перезапустите приложение и попробуйте ещё раз.
```

Название ограничено 64 символами, текст — 3500. Короткий текст копируется нативной кнопкой
Telegram; длинный выводится отдельным чистым сообщением, которое можно удалить кнопкой. Панель
`/answers` работает только у открывшего её администратора. Команды из системной темы клиентам
не отправляются.
