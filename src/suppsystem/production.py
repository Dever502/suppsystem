...
def validate_production_compose(config: Mapping[str, Any]) -> None:
    ...
    required_services = {"suppsystem", "postgres", "postgres-provision", "postgres-migrate"}
    ...
    suppsystem = _mapping(services["suppsystem"], name="suppsystem service")
    ...
    suppsystem_environment = _environment(suppsystem, service_name="suppsystem")
    ...
    suppsystem_image = suppsystem.get("image")
    provision_image = provision.get("image")
    migrate_image = migrate.get("image")
    if any(service.get("build") is not None for service in (suppsystem, provision, migrate)):
        raise ValueError("Production services must use published images, not local builds")
    if not isinstance(suppsystem_image, str) or not is_immutable_image_reference(suppsystem_image):
        raise ValueError("Production suppsystem image must use an immutable digest")
    if provision_image != suppsystem_image or migrate_image != suppsystem_image:
        raise ValueError("suppsystem and PostgreSQL one-shot services must use the same image")
    ...
    if suppsystem_environment.get("POSTGRES_ADMIN_PASSWORD") not in {None, ""}:
        raise ValueError("suppsystem must not receive the PostgreSQL bootstrap password")
    if suppsystem_environment.get("MIGRATION_DATABASE_URL") not in {None, ""}:
        raise ValueError("suppsystem must not receive the PostgreSQL migration credential")
    if str(suppsystem_environment.get("MIGRATIONS_AT_STARTUP", "")).casefold() != "false":
        raise ValueError("production suppsystem must delegate migrations to postgres-migrate")
    ...
    _validate_database_url(
        suppsystem_environment.get("DATABASE_URL"),
    ...
    dependencies = _mapping(suppsystem.get("depends_on"), name="suppsystem depends_on")
    postgres_dependency = _mapping(
        dependencies.get("postgres"), name="suppsystem postgres dependency"
    )
    provision_dependency = _mapping(
        dependencies.get("postgres-provision"),
        name="suppsystem postgres-provision dependency",
    )
    migrate_dependency = _mapping(
        dependencies.get("postgres-migrate"),
        name="suppsystem postgres-migrate dependency",
    )
    if postgres_dependency.get("condition") != "service_healthy":
        raise ValueError("suppsystem must wait for a healthy postgres service")
    if provision_dependency.get("condition") != "service_completed_successfully":
        raise ValueError("suppsystem must wait for successful PostgreSQL role provisioning")
    if migrate_dependency.get("condition") != "service_completed_successfully":
        raise ValueError("suppsystem must wait for successful database migrations")
