...
При старте бот создаёт отдельную системную тему `📚 Готовые ответы` и базовую группу `Общее`.
Каталог общий для всех администраторов. Только внутри этой темы доступны:

| Команда | Действие |
| --- | --- |
| `/addgroup Название группы` | создать группу и один раз опубликовать её карточку |
| `/addanswer Группа \| Название` + текст со следующей строки | сохранить ответ в выбранной группе и один раз опубликовать его карточку |
| `/answers` | сначала открыть список групп, затем ответы выбранной группы |

Пример создания:

```text
/addgroup НЕ РАБОТАЕТ AI

/addanswer НЕ РАБОТАЕТ AI | AI не отвечает
Понимаем проблему. Перезапустите приложение и попробуйте ещё раз.
```

Название группы и ответа ограничено 64 символами, текст — 3500. Названия групп уникальны;
одинаковое название ответа разрешено в разных группах, но не внутри одной группы. После
обновления ранее созданные ответы автоматически находятся в группе `Общее`.

`/answers` показывает группы и ответы постранично. Короткий текст копируется нативной кнопкой
Telegram; длинный выводится отдельным чистым сообщением, которое можно удалить кнопкой. Данные
каталога доступны всем администраторам, а конкретной временной панелью может пользоваться только
открывший её администратор. Команды из системной темы клиентам не отправляются.
