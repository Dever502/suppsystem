...
ID в `system_settings`. Команды `/addgroup`, `/addanswer` и `/answers` обрабатываются только
внутри этой темы, доступны администраторам и никогда не проходят через доставку клиенту.

`/addgroup` создаёт явно названную группу. `/addanswer` принимает первую строку в формате
`Группа | Название`, а остальные строки — как текст ответа. Группа должна существовать заранее:
это не позволяет опечатке незаметно создать новый раздел. Названия групп уникальны после
нормализации; названия ответов уникальны только внутри своей группы.

Группы хранятся в `quick_reply_groups`, ответы ссылаются на них через обязательный `group_id`.
Миграция создаёт базовую группу `Общее` и переносит в неё все ответы, сохранённые до появления
групп. Уникальные source chat/message делают повтор одного Telegram update идемпотентным.
После commit в системной теме один раз публикуется постоянная карточка группы или ответа и
сохраняется её `published_message_id`. Как и для других внешних Telegram-вызовов, сбой между
отправкой и commit редко может дать дубль при повторе.

`/answers` создаёт временную inline-панель с двумя уровнями и независимой пагинацией: сначала
группы, затем ответы выбранной группы. Сам каталог общий для всех операторов, но callback панели
принимается только от открывшего её администратора. Выбор ответа показывает preview. Для текста
до 256 символов используется нативная Telegram-кнопка копирования; длинный текст отправляется
отдельным чистым сообщением с кнопкой удаления.
