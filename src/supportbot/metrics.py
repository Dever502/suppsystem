...
class MetricsRegistry:
    ...
    async def render(self, database: Database, runtime_health: RuntimeHealth) -> str:
        ...
        lines = [
            "# TYPE support_queue_depth gauge",
            f'support_queue_depth{{queue="delivery"}} {int(delivery_depth or 0)}',
            f'support_queue_depth{{queue="notification"}} {int(notification_depth or 0)}',
            "# TYPE support_queue_oldest_age_seconds gauge",
            'support_queue_oldest_age_seconds{queue="delivery"} '
            f"{self._age(now, delivery_oldest)}",
            'support_queue_oldest_age_seconds{queue="notification"} '
        ...
        for (component, outcome), value in sorted(self._events.items()):
            lines.append(
                f'support_events_total{{component="{component}",outcome="{outcome}"}} {value}'
            )
        lines.append("# TYPE support_external_request_duration_seconds summary")
        for component, outcome in sorted(self._latency_count):
            labels = f'component="{component}",outcome="{outcome}"'
            lines.append(
                "support_external_request_duration_seconds_count"
                f"{{{labels}}} {self._latency_count[(component, outcome)]}"
            )
            lines.append(
                "support_external_request_duration_seconds_sum"
                f"{{{labels}}} {self._latency_sum[(component, outcome)]:.6f}"
            )
        lines.append("# TYPE support_heartbeat_age_seconds gauge")
        for component, age in runtime_health.progress_ages(now=time.monotonic()).items():
            lines.append(f'support_heartbeat_age_seconds{{component="{component}"}} {age:.6f}')
        return "\n".join(lines) + "\n"
