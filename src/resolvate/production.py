...
def validate_production_compose(config: Mapping[str, Any]) -> None:
    ...
    required_services = {"resolvate", "postgres", "postgres-provision", "postgres-migrate"}
    ...
    resolvate = _mapping(services["resolvate"], name="resolvate service")
    ...
    resolvate_environment = _environment(resolvate, service_name="resolvate")
    ...
    resolvate_image = resolvate.get("image")
    provision_image = provision.get("image")
    migrate_image = migrate.get("image")
    if any(service.get("build") is not None for service in (resolvate, provision, migrate)):
        raise ValueError("Production services must use published images, not local builds")
    if not isinstance(resolvate_image, str) or not is_immutable_image_reference(resolvate_image):
        raise ValueError("Production resolvate image must use an immutable digest")
    if provision_image != resolvate_image or migrate_image != resolvate_image:
        raise ValueError("resolvate and PostgreSQL one-shot services must use the same image")
    ...
    if resolvate_environment.get("POSTGRES_ADMIN_PASSWORD") not in {None, ""}:
        raise ValueError("resolvate must not receive the PostgreSQL bootstrap password")
    if resolvate_environment.get("MIGRATION_DATABASE_URL") not in {None, ""}:
        raise ValueError("resolvate must not receive the PostgreSQL migration credential")
    if str(resolvate_environment.get("MIGRATIONS_AT_STARTUP", "")).casefold() != "false":
        raise ValueError("production resolvate must delegate migrations to postgres-migrate")
    ...
    _validate_database_url(
        resolvate_environment.get("DATABASE_URL"),
    ...
    dependencies = _mapping(resolvate.get("depends_on"), name="resolvate depends_on")
    postgres_dependency = _mapping(
        dependencies.get("postgres"), name="resolvate postgres dependency"
    )
    provision_dependency = _mapping(
        dependencies.get("postgres-provision"),
        name="resolvate postgres-provision dependency",
    )
    migrate_dependency = _mapping(
        dependencies.get("postgres-migrate"),
        name="resolvate postgres-migrate dependency",
    )
    if postgres_dependency.get("condition") != "service_healthy":
        raise ValueError("resolvate must wait for a healthy postgres service")
    if provision_dependency.get("condition") != "service_completed_successfully":
        raise ValueError("resolvate must wait for successful PostgreSQL role provisioning")
    if migrate_dependency.get("condition") != "service_completed_successfully":
        raise ValueError("resolvate must wait for successful database migrations")
