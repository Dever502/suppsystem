...
исходный текст и список тегов записываются в `quick_responses`, а бот отвечает на него
коротким статусом `[SAVE]`.
...
`edited_message` до дедлайна переводит ту же запись в `valid`, отменяет задачу, удаляет
предупреждение, заменяя его на `[SAVE]`. Повторные невалидные правки не продлевают исходный
дедлайн. Для одного ответа хранится и обновляется только одно служебное reply-сообщение.
