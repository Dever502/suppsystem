...
marker=${deploy_dir}/.supportbot-drill-environment
baseline_image=$1
candidate_image=$2
report=$3
...
running=$(compose ps --status running --services supportbot)
[ -z "$running" ] || {
    echo "supportbot unexpectedly restarted after failed restore" >&2
    exit 1
}
