...
закреплённую панель с двумя действиями:

- `📖 Выбрать готовый ответ` — открыть группы, затем ответы выбранной группы;
- `➕ Добавить готовый ответ` — запустить пошаговое добавление без синтаксиса команд.

При добавлении оператор выбирает существующую группу либо нажимает `Создать новую группу`, затем
обычными сообщениями отправляет короткое название кнопки и полный текст. Перед записью бот
показывает предпросмотр с кнопками сохранения, изменения и отмены. После сохранения доступны
копирование, добавление следующего ответа и переход в выбранную группу.

Группы показываются с количеством ответов, `Общее` располагается первой, остальные группы и
ответы сортируются по названию. Оба списка имеют пагинацию. Временный мастер привязан к
открывшему его администратору, удаляет введённые сообщения после обработки и автоматически
закрывается через 10 минут бездействия. Сам каталог остаётся общим для всех администраторов.

Название группы и ответа ограничено 64 символами, текст — 3500. Названия групп уникальны;
одинаковое название ответа разрешено в разных группах, но не внутри одной группы. Ранее
созданные ответы автоматически находятся в `Общее`.

Короткий текст копируется нативной кнопкой Telegram; длинный выводится отдельным чистым
сообщением с кнопкой удаления. Команды из системной темы клиентам не отправляются.

Старые команды `/addgroup`, `/addanswer` и `/answers` сохранены только как технический fallback;
для обычной работы используйте закреплённую панель.
