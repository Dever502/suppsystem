...
case "$backend" in
    sqlite)
        compose run --rm -T --no-deps supportbot \
            python -m supportbot.operations sqlite-validate < "$input"
        ;;
    postgres)
...
restore_exit() {
    ...
    if [ "$application_stopped" = true ] && [ "$status" -ne 0 ]; then
        echo "Restore failed; supportbot remains stopped for manual verification" >&2
    fi
    exit "$status"
}
...
compose stop supportbot
application_stopped=true

if [ "${SUPPORTBOT_RESTORE_FAILURE_INJECTION:-}" = "after_stop" ]; then
...
case "$backend" in
    sqlite)
        compose run --rm -T --no-deps supportbot \
            python -m supportbot.operations sqlite-restore < "$input"
        ;;
    postgres)
...
if [ "$backend" = "postgres" ]; then
    ...
    # one-shot container so Compose must execute migrations again before supportbot.
