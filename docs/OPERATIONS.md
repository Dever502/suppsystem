...
сообщение. В нём можно указать от 0 до 4 любых Telegram-хештегов. Бот сохраняет исходный текст
без переоформления и отвечает на него коротким сообщением `[SAVE]`.

Поиск выполняется штатной лупой Telegram по тексту или хештегу. Команды, группы, кнопки,
персональные каталоги и Inline Mode не используются.

Если в сообщении больше четырёх хештегов, бот отвечает непосредственно под ним:
`⚠️ Неправильное количество тегов. Укажите не более 4 тегов, иначе сообщение будет удалено
через 5 минут.` Предупреждение не содержит обратного отсчёта и не редактируется. Оператор может
исправить исходное сообщение: бот обрабатывает edit, удаляет предупреждение, сохраняет новую
версию и заменяет предупреждение на `[SAVE]`. Если нарушение не исправлено, через пять минут
удаляются исходное
