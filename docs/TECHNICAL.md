...
обновляет и закрепляет в ней постоянную кнопочную панель. ID темы и панели хранятся в
`system_settings`. Если сообщение панели удалено, следующий запуск создаёт его заново.

Кнопки постоянной панели общие, но не редактируют её. Нажатие создаёт временное меню конкретного
администратора. Каталог имеет два уровня с независимой пагинацией: группы с количеством ответов,
затем ответы выбранной группы. `Общее` выводится первой, остальные элементы сортируются по
нормализованному названию. Callback временного меню принимает только открывший его администратор.

Добавление реализовано как пошаговая in-memory сессия администратора. Оператор выбирает группу
или создаёт новую, после чего Telegram `ForceReply` последовательно запрашивает название кнопки
и текст. Введённые сообщения и подсказки удаляются, перед commit показывается preview с
сохранением, исправлением и отменой. Сессия автоматически удаляет временные сообщения через
10 минут; незавершённый черновик также ожидаемо теряется при рестарте и в БД не попадает.
...
Preview использует нативное копирование для текста до 256 символов. Длинный текст отправляется
отдельным чистым сообщением с кнопкой удаления. `/addgroup`, `/addanswer` и `/answers` остаются
скрытым fallback и, как весь кнопочный сценарий, обрабатываются только внутри системной темы;
ни одно такое сообщение не проходит через доставку клиенту.
