...
from supportbot.config import Settings, get_settings
...
from supportbot.logging_config import configure_logging
...
from supportbot.remnawave import RemnawaveClient
from supportbot.runtime_health import RuntimeHealth
from supportbot.runtime_supervision import shutdown_runtime, supervise_ingress
...
async def validate_support_group(bot: Bot, support_group_id: int) -> None:
    ...
    if member_status not in {"administrator", "creator"}:
        errors.append("support bot must be an administrator in SUPPORT_GROUP_ID")
    if member_status == "administrator" and can_manage_topics is not True:
        errors.append("support bot must have permission to manage topics")
